...
resource "azurerm_linux_web_app" "linux_web_app" {
  ...
  site_config {
    always_on                                     = lookup(try(var.configuration.site_config, {}), "always_on", true)
    container_registry_managed_identity_client_id = lookup(try(var.configuration.site_config, {}), "container_registry_managed_identity_client_id", null)
    container_registry_use_managed_identity       = lookup(try(var.configuration.site_config, {}), "container_registry_use_managed_identity", false)
  }
